...
- [~] **IN PROGRESS.** **Step 1 — graceful-degradation foundation ✅ done 2026-06-29.** New `firebase-config.js` (placeholder config, **`FIREBASE_ENABLED = false` by default** — no secrets, runs 100% local out of the box) + Firebase compat CDN `<script>`s in the shell + new `src/07-sync.js` (`Sync` module). `Sync.init()` (called in DOMContentLoaded) boots Firebase **only if** enabled + real keys + SDK loaded; otherwise it's a **no-op and the app stays fully local** (this is what preserves the offline/clone-and-run/`file://` ethos and keeps the harness green). Verified: harness **66/66** with a new smoke check proving `Sync.isEnabled() === false` when the SDK is absent, app byte-behavior identical, 0 errors. Test harness now `fulfill`s `firebasejs` requests empty (regex route) so tests are deterministic/offline. SW `PRECACHE` += `firebase-config.js` + `src/07-sync.js`; cache → v85.
  - **Remaining P3 steps (not yet built — need a real Firebase project to verify; can't be tested headless since the SDK is blocked):** anonymous auth + `onAuthStateChanged`; RTDB IndexedDB offline persistence; a `Store` cloud-mirroring layer (localStorage stays the offline cache, mirror writes → `characters/{id}`, listen for down-sync); **first-run local→cloud roster migration** (with data-loss guardrails — never overwrite without confirm, keep the local backup); optional **Google account linking** in a settings/menu action. Schema: `characters/{characterId}: { owner:uid, campaignId, …fields…, rolls, journal }`.
