...
## ⭐ STATUS DASHBOARD — read this first (updated 2026-08-20)
...
- **SW cache `tor2e-v126`** · git repo (origin = github.com/arti47/tor2e-tracker, branch main) · shells (`character-tracker.html` = `index.html`) in sync.
...
| **8** | **Fix the 16 playtest findings** (2026-09-12) | **M** | **The live backlog, and the highest-value work outstanding.** A played session found 16 issues no existing spec catches — full list + quoted app text + beat in `docs/playtest-2026-09-12.md`, summary in the dashboard entry above. Fix order should be by how badly each blocks *play*: (a) the ▶ Play ↔ Journey dead-end, (b) the Prowess TN loss (a **silent wrong number on real rolls** — the only correctness bug in the set), (c) the FP-wizard double-recovery, (d) no way to end a saga, (e) the missing/unreachable controls. Each fix wants a spec check built **from what the app actually produces** — a fixture assembled by hand can prove a function works while the path to it stays impossible. |

**Recommended order: 8 (playtest fixes) → 1 (deploy) → 2 (table test). 5 and 7 only if a real need emerges. Items 3, 4, 6 ✅ done 2026-07-02.** Deploying before 8 would ship a Play tab that dead-ends.
...
- **`sw.js` `CACHE_VERSION`**: `tor2e-v126` (bump on every deploy). `PRECACHE` lists all 8 `src/*.js` + `vendor-qrcode.js` + `styles.css` + `firebase-config.js` + shells + PWA assets — **add any new file there**. SW strategy: HTML/navigations network-first; static assets cache-first; auto-activate.
- **Test harness**: `npm test` → 7 specs / **247 checks** (smoke 23, adversaries 11, ux 159, spillage 20, a11y 7, gm 20, **reachability 7**). A fresh clone needs `npm install` first (`playwright-core` is the only devDependency; `tests/browser.js` glob-resolves a cached Chromium, `CHROMIUM_BIN` overrides). Cloud paths are no-ops in tests (SDK blocked) — verify live features via preview against the real project.
