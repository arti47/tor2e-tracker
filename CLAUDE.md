...
## ⭐ STATUS DASHBOARD — read this first (updated 2026-09-12)
...
- **SW cache `tor2e-v127`** · git repo (origin = github.com/arti47/tor2e-tracker, branch main) · shells (`character-tracker.html` = `index.html`) in sync.
...
| **9** | **Fix the run-2 playtest findings** (2026-09-12) | **M** | **Blocks deploying.** 17 findings + 13 minors in `docs/playtest-2-2026-09-12.md`. Order by what stops play: **(a)** the Fellowship Phase one-shot regression — `fpComplete` nulls `fpWizardState` then `fpClose`→`fpPersist` restores it, so no hero can ever take a second phase; **(b)** FP step 3's Spend XP opens behind the wizard (z-index) and the fallback route enforces no caps; **(c)** a Valour/Wisdom rank-up not spent in the moment is lost forever; **(d)** Clash spend regressed — renderer runs, container still hidden; **(e)** Journey Events unreachable from ▶ Play; **(f)** pregen Patron keys match no table; **(g)** Eye Awareness only rises on Dice-tab rolls. **Every fix here needs a check that asserts the player-visible OUTCOME, not the mechanism** — see GOTCHA 20, which exists because three run-1 fixes passed specs while staying broken in play. |

**Recommended order: 9 (run-2 fixes) → 1 (deploy) → 2 (table test). 5 and 7 only if a real need emerges. Items 3, 4, 6 ✅ done 2026-07-02, item 8 ✅ done 2026-09-12.** Item 8 closed 2026-09-12, but run 2 opened item 9 — deploying now would ship an app where no hero can take a second Fellowship Phase.
...
- **`sw.js` `CACHE_VERSION`**: `tor2e-v127` (bump on every deploy). `PRECACHE` lists all 8 `src/*.js` + `vendor-qrcode.js` + `styles.css` + `firebase-config.js` + shells + PWA assets — **add any new file there**. SW strategy: HTML/navigations network-first; static assets cache-first; auto-activate.
- **Test harness**: `npm test` → 7 specs / **265 checks** (smoke 24, adversaries 11, ux 174, spillage 22, a11y 7, gm 20, **reachability 7**). A fresh clone needs `npm install` first (`playwright-core` is the only devDependency; `tests/browser.js` glob-resolves a cached Chromium, `CHROMIUM_BIN` overrides). Cloud paths are no-ops in tests (SDK blocked) — verify live features via preview against the real project.
...
- **localStorage keys**: a **multi-character roster** (added 2026-05-31):
  ...
  - *(the 2026-09-12 playtest-fix pass added no keys either — the two new fields live ON the character, so they export and sync: **`char.tnAdjust`** `{str,hrt,wit}` (standing Attribute-TN adjustments — Prowess −1, Lifepath Major-Event +1; see GOTCHA 19) and **`char.fpWizardState`** (the in-flight Fellowship Phase, so closing the wizard pauses instead of resetting). `char.journey.pendingEventRoll` and `char._moriaPrev` are likewise on the character.)*
