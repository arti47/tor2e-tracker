...
- [ ] **Adversary-attack-TN RAW fix — DEFERRED (do after P1, needs notebook verification).** Split out from the content port deliberately: `foeAttackHero` currently computes `tn = f.atkTN + heroParry`, whereas the loremaster's documented RAW says an adversary's attack TN vs a hero = the **hero's Parry alone**, with the target's stance applied as a **±Success die** (Forward +1d / Defensive −1d), not a Favoured/Ill Feat die. This changes **hit probabilities**, so it must land **after P1** (test net) and be re-verified against the NotebookLM Rulebook before touching combat math. Target: `foeAttackHero` (and the stance handling in `heroAttackFoe`). Risk: medium — it is a correctness fix toward RAW, not a rules change, but unguarded combat-math edits are exactly what P1 exists to protect.
...
- [ ] **SDK, config, anonymous auth, offline persistence, Store cloud mode, character migration, Google linking.**
...
- [ ] **GM dashboard, gated by `role === "loremaster"` (or a GM toggle).**
...
- [ ] **Keyboard + screen-reader support (port Dragonbane's a11y work).**
...
- [ ] **U11 — Big-screen table mode.** Goal: cast to a TV at an in-person table (loremaster roadmap). Target: a new high-contrast, large-text party/encounter view. Behavior: read-only glanceable vitals/initiative for the room. **(do after P2 — pairs naturally with the P6 GM party panel)**
