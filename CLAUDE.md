...
## ⭐ STATUS DASHBOARD — read this first (updated 2026-09-14)
...
- **SW cache `tor2e-v129`** · git repo (origin = github.com/arti47/tor2e-tracker, branch main) · shells (`character-tracker.html` = `index.html`) in sync.
...
- **Newcomer onboarding pass (2026-08-20, SW v102, harness 115/115):** audited the app as someone who owns neither rulebook nor solo-RPG experience, then closed every gap found. Measured before/after: **tab intros 9/14 → 14/14**, **(?) hints 9 (Character only) → 23 across 5 tabs**, Reference groups 6 → 7. Six changes, all additive — **no rules, schema, or dice-math change**:
  - **A — first run.** `maybeOfferTutorial` stamped `offered = true` *before* awaiting the dialog, so a dismissal (or a reload inside its 700 ms delay) destroyed the only proactive onboarding permanently; it is now stamped only after an answer. Buttons are "📖 Start the tutorial" / "Not now" (was OK/Cancel). New **`renderNewcomerBanner()`** puts a dismissible "👋 Start here" card on the Character tab whenever the active hero has no culture (`newcomerNeedsHelp()`), offering tutorial / Build tab / pre-gen hero, and naming solo play. Re-rendered from `render()` and on tab switch.
  - **B — hints everywhere.** `initHintButtons()` was hard-scoped to `#panel-character .counter-label`. Now it also binds **`data-hint="Term"` on any element, any tab**, via idempotent `_attachHint`; `hintRow()` resolves one shared vocabulary across `REFERENCE.terms`/`tn`/`conditions`/`solo`/`combatTasks` + `STANCE_INFO`, and renders no (?) that would open nothing. Hints now use `alertStyled` (was native `alert`).
  - **C — the Reference tab had ZERO solo content** (verified: Oracle/Telling/Lore/Eye/Chronicle/Moria/Fortune all absent). Added a **`REFERENCE.solo`** group (10 entries) incl. **"How a solo session runs"** — the six-step loop (open a scene → act → ask the Oracle → roll → write it down → repeat) that nothing in the app taught. Also added 10 Key Terms that appear on-screen but were never defined (Favoured, Ill-Favoured, Inspired, Fellowship Focus, Support, Stance, Resistance, Time Limit, Standard of Living, the two phases) and 7 Moria/Band terms (Readiness, Dispositions, Disposition Focus, Burden, Hardened, Clash, the Band itself).
  - **D — tab intros** for the 5 tabs that opened with no explanation at all: character, combat, **journey (had no `.hint` anywhere)**, dice, chronicle. New `.tab-intro` style (dashed, quieter than a real card).
  - **E — blank-hero guards.** `rollDice()` on an unbuilt hero silently printed `9 / 3 / 12 vs TN 15 — FAIL` against placeholder attributes; it now warns **once** (`_blankRollWarned`) and still rolls. `startJourney`/`startCouncil`/`startSkillEndeavour` refuse with a pointer to Build / pre-gens.
  - **F — the ☰ menu** was a flat 24-item list; now grouped under 7 `.menu-group` headers (New here? · Play on your own · Heroes · Play with others · Appearance · Backup & restore · Danger zone), with a "📖 Rules Reference" entry (`openReferenceTab()`). The solo toggles read **"🗡️ Solo Play: Strider Mode"** (was "Enable Strider Mode (solo)"), and both enable-dialogs now lead with *what solo play is* before the stat deltas.
  - +11 ux-spec checks (banner show/hide, hint attach + idempotence + lookup, 5 intros, warn-once, menu grouping, solo label, Reference solo group).
- **Onboarding pass 2 — the deep sweep (2026-08-20, SW v103, harness 119/119):** the first pass left the jargon-densest surfaces thin — Band carried 66 controls behind 4 hints, Battle 16 behind 1. Closed that and finished the sweep app-wide. **(?) hints 23 → 45, now on 13 of 14 tabs** (was 5); **`.tab-intro` on all 14** (Band and Battle gained proper ones). Added **28 more `REFERENCE.solo` entries** covering the whole Moria/Battle vocabulary (Mission, Hunt Threshold, Damage Threat, Band Endurance/Fatigue Tests, Ally Gift/Quirk, Kingly Gift, Desperate Stand, Hero-or-Band, Shared Calling, War Party, Might, Foe Resistance, Archfoe, Leader Focus, Band Stance, Advantage/Complication, Battlefield Aspect, Moria Fellowship Phase) plus the creation/journey/gear terms the other tabs needed (Heroic Culture, Calling, Distinctive Feature, Previous Experience, Skill/Adventure Points, Marching Test, Journey Event, Blessing, Scene). +4 ux-spec checks, incl. a **dead-hint guard**: every `data-hint` in the document must resolve to real text, so a (?) can never open nothing.

- **Sequence-of-play pass (2026-08-20, SW v104, harness 128/128):** audited every button on the solo surfaces against the order you actually play in. The guards were already there (`enduranceTest`/`fatigueTest`/`beginBattle` all blocked without allies) — the fault was **card order**, so tabs didn't read top-to-bottom. Seven fixes, all reorder/gating, no rules change:
  - **Band tab reordered to the real sequence** and numbered on screen: **1 · Allies → 2 · Mission Planning → 3 · Band of Allies → 4 · Dispositions** → Tests → Solo Tools → Fellowship Phase. Allies was card **7 of 8** despite being the mandatory first action (Mission's roster picker needs them; Tests and Battle hard-block without them).
  - **Oracle tab**: Telling + Lore lead (what you reach for), then the Moria generators, then Fortune/Ill-Fortune under a *"Reactive — you rarely come here for these"* banner (a Rune/Eye on any roll already auto-prompts them), then History.
  - **Chronicle**: leads with the write box, not the Tale-of-Years editor; the clock moves to the bottom as the bookkeeping tool it is, with a tappable `#ch-date-top` stamp keeping the date visible.
  - **Eye of Mordor** moved from card 8 to directly under **Hope** (Shadow drives EA, so they read together) + a new **`#eye-pill` header pill** (`👁 n/threshold`, ⚠ at the Hunt threshold) visible from every tab in solo, tap to jump. **Bug found doing this:** `refreshEyeOfMordor()` early-returns when `!isSolo()`, so the pill would have gone stale after leaving solo mode — it now hides on that path (spec-pinned).
  - **XP scheme**: session XP and Milestone XP are alternatives, but both buttons were always live. `refreshXpMode()` now shows only the one `char.experienceMode` names, with a *switch* link (`switchXpMode()`).
  - **Fellowship Phase**: two entry points with no signposting (Character-tab core wizard vs the Band-tab Moria Hurried/Brief/Extended card — Moria has no Yule). `refreshFpEntry()` hides the core wizard in Moria mode and points at the Band tab instead.
...
- **Idiot-proofing sweep 2 — modals & guards (2026-08-20, SW v106, harness 146/146):** re-audited the *current* build for a player who owns no rulebook. The tab surfaces held up (14 tabs, 54 rendered `(?)` hints, every tab self-explains); the gaps were on the two surfaces never audited before — **the 23 overlay modals** and **the guard messages**.
  - **8 more dead-end guards converted to `requireStep()`** (last pass fixed only the 3 Band/Battle ones): no-weapon attack → jumps to War Gear, queued journey event → jumps to the Journey card, no scheduled event, First Aid when not Wounded, random name with no culture → Build, plus better copy on the FP phase-type and the Council/Endeavour setup prompts. Each now says what is missing *and* offers the trip.
  - **Picker modals printed bare notation.** Weapon picker showed `Dmg 4 · Inj 16 · Load 2`, armour `3d · Load 9 · min: Common`, bestiary `End 12, Parry 3 … sword 2d (5/16)` — none of it defined anywhere. Added an inline legend to each, plus **7 new `REFERENCE.terms`** (Damage, Injury, Protection, Might, Hate, Adversary stat line, and the 1h/2h Injury split) so the same words resolve through the `(?)` system.
  - **Verified NOT broken** (checked, no change needed): all 23 overlays have a visible exit — `new-reward`/`new-virtue` looked trapped in the scan but carry a *"Skip for now"* close button and honour Escape; the `window.alert` monkey-patch still routes all 72 remaining native `alert()` calls through `alertStyled`, so none render unstyled; empty states across the app already name the next action; `applyBackstory` confirms before overwriting.
  - **Council + Skill Endeavour cannot be started unconfigured** — their setup rows ship with a selected default, so those two guards are unreachable. Pinned that as the stronger invariant instead of the guard.
  - +9 ux-spec checks, incl. an **overlay no-trap guard** (every overlay must expose a visible way out) and a **no-bare-alert guard** on the wrong-order paths.

- **Hint parity + the dead-hint guard (2026-08-20, SW v107, harness 153/153):** closed the last thin surfaces flagged in the previous sweep. Rendered `(?)` hints **54 → 70**; Combat **1 → 9**, GM **2 → 5**, and every jargon tab now has at least one.
  - **Real defect found:** `data-hint="Stance"` resolved to **nothing**. `STANCE_INFO` is keyed lowercase (`forward`), `hintRow()` looked up the term verbatim, and `_attachHint` renders no button at all for an unresolvable term — so the Combat tab's only hint had been silently absent with no error anywhere. `hintRow()` now lower-cases before the STANCE_INFO lookup.
  - **17 new `REFERENCE.terms`** covering the vocabulary those tabs print: Stance, Engaged Foes, Foe Parry, Parry Total, Opening Volley, Encounter, Skill, Combat Proficiency, Hexes, Journey Role, Roleplay bonus, Scene, Tale of Years, Eye of Mordor, NPC Ledger, Combat Action Deck, Group Shadow Test.
  - **New spec guards make this class of bug impossible to reintroduce:** every `data-hint` in the document must resolve via `hintRow()` **and** must actually render a `.hint-q` button; the stance lookup is pinned for `Stance`/`Forward`/`forward`; and no jargon tab may drop to zero hints. (Reference is itself the glossary and Gear is free text — both exempt by design.)

- **Tutorial audit — the sandbox could strand you (2026-08-20, SW v108, harness 158/158):** first end-to-end audit of the tutorial itself, the app's primary teaching vehicle. Content is sound (10 lessons / 57 steps, every step's tab + selector resolves, every step has copy) — but the sandbox had a **data-confusion bug**:
  - **`window._tutSandbox` was memory-only.** Open the tutorial, get distracted, close the app → you reopen **as "<name> (Practice)"** with your real hero inactive in the roster and nothing explaining why. A newcomer would reasonably conclude their character was lost. Reproduced: real hero *Aragorn* → reload → active hero *Beran (Practice)*.
  - **Fix:** the sandbox is now persisted to **`tor2e-tut-sandbox`**, and a new **`_tutRecoverSandbox()`** runs at boot (before `maybeOfferTutorial`): it explains what happened, names the practice hero and the real one, then routes into the normal `_tutExitSandbox()` Keep/Discard choice. Cleared on any proper exit.
  - **Tutorial menu now answers Escape** like every other overlay — its "Done — exit tutorial" button gained `.close`, so the P8 Escape handler finds it (and therefore unwinds the sandbox rather than doing nothing).
  - **Verified NOT broken** (probe artifacts, no change made): the spotlight frame *does* highlight every step that has a selector — an earlier reading was taken synchronously before the `requestAnimationFrame` placement ran; steps without a `sel` centre the card by design. Counter bounds also hold under abuse — Endurance floors at 0 and ceilings at max, Shadow+Scars caps at Max Hope, Valour floors at 0, non-numeric journey input is rejected with a message, and none of it throws.
  - +5 ux-spec checks: lesson/step counts, every step resolving its tab + selector + copy, sandbox persistence, and that discarding the practice hero restores the real one.

- **Output & progress pass (2026-08-20, SW v109, harness 163/163):** audited the most-repeated thing a player looks at — the roll result — plus creation progress. Three fixes, one of them self-inflicted:
  - **The Build checklist lied.** The previous pass retitled a *static* list of reminders to "📋 Your progress — what's left", promising tracking it never did. Replaced with a real **`renderBuildChecklist()`**: 11 derived steps, a ✓/○ per item, a progress bar, per-item jump links to the tab that satisfies it, and a "your hero is ready" close. Re-rendered from `render()`. **Blank-hero trap avoided:** `DEFAULT_CHARACTER` already carries placeholder attribute ratings, so the Attribute-set row is gated on a culture being applied too — otherwise a brand-new hero showed 1/11 done.
  - **Dice were three unnamed coloured boxes.** `.feat-die` and `.success-die` differ only by styling — nothing said which was which, and a screen reader read bare digits. New `_labelFeatDie()` / `_labelSuccessDie()` add `title` + `aria-label` at all **three** render sites (dice tab, inline rolls, protection rolls), naming the Eye and the Gandalf rune explicitly. *Ordering matters:* the label must be applied AFTER `textContent`, or it captures an empty value — the first attempt did exactly that.
...
- **Glossary audit — the explanations themselves (2026-08-20, SW v111, harness 173/173):** every prior pass *added* explanatory text; this one audited the text for the classic rulebook-less failure — **defining jargon with more jargon** — and for damage done by six passes of accretion.
  - **Duplicate entries found and removed.** Separate passes independently added **Stance** and **Eye of Mordor**, and `Scene (Chronicle)` / `Scene` were near-duplicates. The Reference tab was rendering each **twice, worded differently**, and `hintRow()` silently returned whichever came first. Kept the fuller wording of each; `Scene (Chronicle)` was renamed to `Scene` and its `data-hint` updated. (`Might` vs `Might (foe)` are genuinely different concepts — adversary Might vs Battle Might — and were left.)
  - **8 core terms other definitions leaned on were never defined:** **Reward, Virtue, Success die, Piercing Blow, Resolve, Combat Task, Eye Awareness, Revelation Episode**. Reward and Virtue are the worst of these — they are the payoff of every Valour/Wisdom rank-up and were used in four definitions while being explained nowhere. Definitions leaning on undefined jargon: **23 → 9**, and the 9 are matcher artifacts ("success dice" plural vs the term *Success die*; "rune", which the Feat-die entry explains inline).
  - **The dead-hint guard earned its keep**: renaming the `Scene` key broke `data-hint="Scene (Chronicle)"` and the spec failed immediately rather than silently dropping a `(?)`.
  - **New spec guards**: no duplicate glossary terms (keyed case-insensitively across all five groups), the core vocabulary above must resolve, no stub definitions under 25 characters, and total coverage may not regress. Glossary now **~95 entries / 20.7k characters** across 7 groups.

- **Solo-voice pass (2026-08-20, SW v112, harness 177/177):** opened the three surfaces flagged as unexamined in earlier passes — the **FP wizard's four steps**, and the copy a solo player meets mid-flow. The wizard itself is sound (numbered steps, explanatory prose, live previews like *"Current: 6/12 → will become 10 (+4)"*, undertakings marked FREE/Calling). The defect was elsewhere: **a lot of RAW-derived copy assumes a Loremaster and a Company that a solo player does not have.**
  - Examples a soloist actually hits: the Gather Rumours undertaking said *"Receive a rumour from **the Loremaster**"*; the Council intro said *"**the Company** must accumulate successful rolls"*; FP recovery asked *"How effective was **the Company**…"*; the **Hate** and **Combat Action Deck** glossary entries described the Loremaster as a separate person.
  - **New `soloWord(groupText, soloText)` + `applySoloWording()`** (in `src/01-core.js`, hooked into `refreshStriderUI`): markup carries `data-solo-text="…"`, the original wording is stashed in `data-group-text` on first swap, so **toggling solo on and off is lossless**. Data-driven copy (FP undertakings) gained a parallel `descSolo` resolved at *render* time — **not** at load: `FP_UNDERTAKINGS` is a top-level `const`, so calling `soloWord()` inside it froze the wording at boot and never followed a mode toggle. That was caught and fixed before it shipped.
  - **Spec caveat worth keeping:** the guard that flags glossary entries mentioning "Loremaster" must allow the *legitimate* explanatory ones — *"Your stand-in for the Loremaster"* is exactly the right phrasing for a soloist, not a defect. The check only fires when copy assumes someone else is in the room.
  - +4 ux-spec checks: wording swaps on, restores losslessly on toggle-off, Gather Rumours points solo players at the Oracle and groups at the Loremaster, and no glossary entry assumes a Loremaster exists.

- **Sequence-of-play pass 3 (2026-08-20, SW v113, harness 181/181):** re-audited all 14 tabs against play order. Passes 1–2 had landed correctly (Band numbered 1–4, Build 1–9, Advancement earn→spend, GM roster above the group test, dice quick-rolls first). Three ordering issues left, all fixed; two decisions confirmed and left alone:
  - **Council** stacked two subsystems with the *Past Councils* archive wedged between them, so the Endeavour section began halfway down after a history list. Now: Council Setup→Progress→Log, Endeavour Setup→Progress→Log, **Past Councils last**.
  - **Gear** opened on `Useful Items 🔒` — read-only, and empty until you visit Build. Now leads with the editable cards (Travelling Gear → Magical Treasure → Treasure → Notes) and keeps the locked summary at the bottom.
  - **Dice**: moving the quick-roll grid to the top in pass 2 left the manual **🎲 Roll Dice** button ~9k characters below it, behind Combat Tasks and Shadow Tests. Those two are *separate roll triggers*, not part of the manual sequence, so they moved **below the result**. The manual path is now self-contained: quick-rolls → folded manual controls → Hope/Magical/Inspired modifiers → Roll → result → other triggers. (Quick-rolls were never affected — `quickRoll()` ends in `rollDice()` and the result `scrollIntoView`s.)
  - **Confirmed and deliberately unchanged:** **Combat stays play-first** (Stance/Encounter above War Gear/Armour/Shield) — gear is one-time setup that blocks nothing, unlike the Band's allies which hard-block Tests and Battle, so it does *not* follow the Band's setup-first convention. **Conditions stay below Endurance/Hope/Eye** — they read as the consequence of the pools that trigger them, and those cards already carry the auto-warning badges.
  - +4 ux-spec checks pin all three orderings plus the Combat exception, so a later edit can't silently undo them.

- **Sequence-of-play pass 4 — inside the cards (2026-08-20, SW v114, harness 185/185):** tab-level order is now settled across all 14 tabs, so this pass audited order *within* the big composite cards. Three fixes:
  - **Encounter card was inverted for setup.** `+ Add Adversary` rendered **below** the foe list (so the first action of any fight was at the bottom) while `End encounter` sat in the header beside the round counter. Now: round + `Next round ▸` header → **`+ Add Adversary`** → all-foes-attack → foe cards → **`🏁 End encounter`** last.
  - **Moria Fellowship Phase card** bundled four unrelated things with no signposting. Now numbered in play order: **1 · How long do you rest?** → **2 · Was the rest disturbed?** (FP Interruption) → **3 · Spend the time** (undertakings), with **Refresh FP** pulled out below a divider and labelled a *Fellowship Milestone* reward rather than a phase step.
  - **Random Lifepath** interrupted the Build tab's 1–9 numbering. Retitled *"Optional — instead of choosing in step 1"* and its hint now states plainly that running it later overwrites steps 4 and 5. (Position is correct and was kept: it must precede step 2.)
  - **Confirmed, unchanged:** the Dice-tab **Protection Roll** card stays put — the Encounter auto-rolls Protection on a Piercing Blow, so this is the manual fallback and belongs with the other manual rolls.
  - +4 ux-spec checks. Note the existing lifepath check had to be updated with the retitle: it now asserts the card reads as optional **and** names itself an alternative to step 1.

- **Sequence-of-play pass 5 — modifiers and escape hatches (2026-08-20, SW v115, harness 188/188):** audited the *in-progress* cards, which earlier passes never opened. Two findings, one of them a real trap:
  - **`✖ Cancel Journey` was unreachable.** It lived in the **Journey Setup** card, which `renderJourney()` sets to `display:none` the moment a journey starts — so once you began a journey with the wrong hex count there was no way out except `🏁 Arrive at Destination`, which applies arrival effects. Every other subsystem keeps its abort inside the *active* card (`cancelCouncil`, `cancelSkillEndeavour`, `endBattle` all sit last there). The button moved to the bottom of **Journey in Progress** and is relabelled **"✖ Abandon this journey"**; `renderJourney` now shows it `block` rather than `inline-block`.
  - **Pre-roll modifiers rendered *below* the control that consumes them.** In both the Council and Skill Endeavour active cards the skill buttons **fire the roll immediately**, yet `se-roleplay-pick` sat under `se-skill-grid`, and `c-roleplay-pick` + `c-support-btn` sat under `c-interaction-section` — so you had to scroll past the action to set a bonus, then back up to roll. Both blocks moved above their skill grids (`c-support-hint` moved with its button). The **Battle Clash card was already correct** and is the model: focus → stance → gift/Hope modifiers → `🎲 3 · Clash Roll`.
  - **Verified correct, unchanged:** Journey in Progress (Marching Test → Resolve Event → Resolve Peril → Arrive), Quick Build (pick → preview → apply, ×3), and terminal actions sitting at 94–97% depth in every active card.
  - +3 ux-spec checks pin both modifier orderings and the journey abort's reachability.

- **Reachability audit (2026-08-20, SW v116, harness 192/192):** first pass hunting *shipped-but-unreachable* UI rather than ordering or wording. Five classes checked mechanically:
  - **One real defect: Fellowship Focus suppression was never wired.** `#focus-strider-hint` and `#focus-row` both ship in the markup, the hint ships `display:none` — and **no JS file referenced either one**. So the behaviour CLAUDE.md listed as ✅ complete (gap 5 of the Strider audit) never happened: a solo player saw a Fellowship Focus picker that does nothing for them, and the explanatory note was permanently invisible. `refreshStriderUI()` now hides the picker + its Support hint and shows the note, losslessly both ways. The stale ✅ claim above has been corrected in place.
  - **Clean:** 381 `getElementById` targets all resolve (the 17 flagged were `el.id = '…'` assignments or `'prefix-' + n` concatenation — `strider-fortune-action`, `pregen-overlay`, `tut-*`, `toast-wrap`, `fp-step-N`, `skills-<attr>`, `panel-<tab>`). All `requireStep`/checklist/menu jump targets resolve (0 broken). No inert controls — the 26 unreferenced ids are container `<div>`s or buttons carrying inline `onclick`.
  - **Dead code, harmless, left alone:** `toggleDarkMode` (superseded by `cycleTheme`), `oracleSet` (Moria/Strider table choice is made via `isMoria()` at the call site instead), and `ch-combat-card` + its renderer (the Chronicle Combat Log was replaced by the Combat-tab Encounter tracker). 2 orphans out of 592 declared functions.
...
- **Reachability pass 3 — run to zero (2026-08-20, SW v118, harness 203/203):** iterated the detectors until every one returned **0**. Six more classes checked, three fixes, and the dead code cleared out so the sweep is genuinely clean rather than clean-with-known-exceptions.
  - **Two blocking guards converted to `requireStep()`**: applying a **Reward** with no matching gear equipped (`promptApplyReward` — now jumps to War Gear), and firing a **Combat Task** in the wrong Stance (`rollCombatTask` — now jumps to Combat). Both previously named a tab the player then had to find alone.
  - **Lifepath guard rewritten**: *"Pick a Core Rules culture first"* became an explanation that Lifepaths exist only for the six Core cultures and pointing at step 1 on the same tab (no jump needed — the picker is directly above).
  - **Dead code removed** so the detectors report zero: `toggleDarkMode` (superseded by `cycleTheme`), `oracleSet` (the Moria/Strider table choice is made at the call site via `isMoria()`), and `HISTORY_KEY` (superseded by the per-hero `tor2e-rolls-<id>` prefix).
  - **Verified clean, no change needed:** all 36 `data-field` inputs map to real schema keys; 18 skills × 7 pips and 4 profs × 6 pips all render; every culture has cultural virtues; `.moria-only` markup reveals in Moria and hides outside it; all five mode-gated tabs become reachable under their mode; all six Rewards find a target once their gear is equipped; `#b-focus-pick` has no handler at load **by design** — `renderBattle()` binds all four buttons when a battle starts.
  - **Final detector state: orphan functions 0 · unreferenced content constants 0 · blocking guards without a jump 0 · broken jump targets 0.**
  - +6 ux-spec checks lock schema binding, pip rendering, cultural-virtue coverage, mode-gated markup and tabs, and reward targeting.

- **Reachability pass 4 (2026-08-20, SW v119, harness 206/206):** pass-3 detectors re-run and still **0**; seven further classes checked. **No user-facing defect found** — one defensive hardening, and four probe artifacts worth recording.
  - **Hardening:** `fpState` is `null` until `openFPWizard()` runs, so a stray `fpNextStep()`/`fpPrevStep()` threw `Cannot read properties of null`. Both are now guarded (`fpNextStep` falls back to opening the wizard). Not user-reachable — the overlay is only shown by `openFPWizard()`, and Escape clicks `.close` → `fpClose()` — but the throw is gone and spec-pinned.
  - **Verified clean:** every inline `onclick`/`onchange` names a function that exists (0 missing across all 14 tabs in every mode); all 22 overlays close via a **visible** control; every `PRECACHE` entry, manifest icon and shell asset ref exists on disk; every `querySelector('#id')` target exists; the print stylesheet hides no essential sheet panel.
...
- **CI verified against the real runner (2026-08-24):** the workflow ran on GitHub and is **genuinely green — logs checked, not just the badge**: all 7 specs, 213 checks, on a clean checkout (`npm ci` from scratch, no local state). Worth doing, because a green badge over a suite that silently skipped would be worse than no CI. Two things the logs surfaced:
  - **Node 20 deprecation.** GitHub warned that `actions/checkout@v4` / `actions/setup-node@v4` target Node 20 and are already being force-run on Node 24. Moved both to **v5** and the app runtime to **Node 22 LTS**, so this doesn't break when the forcing period ends.
  - **Browser download dominated the run** (25s of 42s). Added `actions/cache` on `~/.cache/ms-playwright`, keyed on `package-lock.json`, with the install skipped on a hit. **Honest cost:** the cache *save* takes ~110s, so the miss run got slower (42s → 2m41s) and it only pays back from the next run on. Measured, not assumed.
  - **A local-vs-CI timing note:** the harness takes ~5s on a GitHub runner vs noticeably longer in a dev container. A suspiciously fast CI run is not by itself evidence the suite skipped — check the per-spec output before concluding anything.
...
- **The saga — how a campaign starts, continues and ends (2026-08-24, SW v120, harness 219/219):** the app's owner, who has read every other pass, said plainly: *"I still don't know how to start playing, sustaining play and ending the game well."* A grep confirmed it — **zero** campaign-arc framing anywhere; `retire` appeared only as the mechanical 4-flaws outcome, never as a way to finish a story. **Fifteen onboarding/sequence passes made every control explain itself and none of them explained the game.** That is the lesson worth keeping: control-level clarity does not add up to knowing how to play.
  - **New `char.saga`** `{started, premise, sessions, adventures, ended, endedHow}` (+ `migrateCharacter` back-fill) and a **`#saga-card`** at the top of the Character tab, deliberately **action-shaped rather than prose** — a 16th block of explanatory text would have repeated the mistake.
  - **Starting:** `sagaBegin()` asks the one thing a character sheet cannot supply — *a reason to leave home* — seeded from the hero's Patron via `PATRON_QUESTS` when there is one. It stamps the premise, opens the first Chronicle scene, and closes with the three-step loop (*act → ask the Oracle or roll → write it down*).
  - **Sustaining:** `sagaStartSession()` recaps the last three timeline entries and restates the premise; `sagaEndSession()` banks session XP and says whether a Fellowship Phase is due. Adventure → Fellowship Phase → adventure is now stated as *the* campaign rhythm.
  - **Ending:** `sagaEndSignals()` surfaces a close only once the hero has earned one (Valour/Wisdom ≥ 5, ≥ 3 Scars, several Fellowship Phases, or retirement). `sagaEnd()` offers the honest TOR2E endings — retire, fall, pass on, or simply stop — writes the last line into a closing scene, and is reversible via `sagaReopen()`.
  - **Two of my own bugs, caught before shipping:** the first draft called a `startScene()` that does not exist (the real API is `ensureActiveScene()` + `pushBlock()`), and `data-hint="Your saga"` resolved to nothing until the glossary entries were written — **the dead-hint guard from the earlier pass failed the build on my own change for the third time.**
  - +6 ux-spec checks covering all three phases, plus 4 new `REFERENCE.solo` entries (Your saga · Starting / Sustaining / Ending a campaign).

- **Saga verified against the rulebooks (2026-08-24, SW v121, harness 221/221):** the saga card shipped as *my* design and was flagged unverified. NotebookLM answers (quoted, cited) confirmed two parts and **corrected three**. This is the first feature in the app built from a guess and then checked — the guess was wrong in a way that mattered.
  - ✅ **Confirmed:** a campaign begins with one hero plus a reason to go — *"missions assigned by a Patron, or … personal goals or problems which compel your character to abandon the comfort of home and hearth"* — and *"introducing a Patron can serve as a great way to kick off a campaign"*. Seeding the premise from `PATRON_QUESTS` is exactly RAW; the tables are explicitly for *"a starting adventure … or the next goal within a continuing campaign"*.
  - ❌ **Wrong — session XP in solo.** Strider Mode advises *against* session-based rewards: *"your sessions might last for a few minutes or a few hours, which can make session-based rewards disconnected from events and achievements in your story"*, and recommends **Experience Milestones** instead. `sagaEndSession()` was auto-awarding session XP. Now: solo defaults `experienceMode` to `'milestone'` on toggling Strider on (unless the player has explicitly chosen, tracked by `char._xpModeChosen`), and session XP is only auto-awarded **out of solo**. The spec that asserted the old behaviour was rewritten to assert the correct one.
  - ❌ **Wrong — Fellowship Phase pacing.** I prompted off an XP threshold. RAW: *"an Adventuring Phase will last two or three sessions of play, followed by a Fellowship Phase"*, each FP *"a minimum of one week, up to an entire season"*, with Yule *"approximately once every three Fellowship Phases"*. New `saga.lastFpSession` (+ migration) resets in `fpComplete()`, and the prompt fires after **2+ sessions**.
  - ❌ **Wrong — end conditions.** My *Scars ≥ 3* signal was invented. RAW gives three endings: death in battle; **succumbing** — *"player-heroes ending the Adventuring Phase with a Shadow score matching their maximum Hope … are retired from the game"* (note: no Flaw requirement, unlike the app's existing 4-Flaws bout rule — these are separate); and **willing retirement**. Moria has its own finish line: *"build a Band of twelve Hardened Allies and undertake a final great mission into the depths"* — computable, since allies already carry a `hardened` flag, so the card now counts them.
  - **Caveat kept:** *12 allies*, *2–3 sessions*, *every 3 FPs* are inline prose figures, not table values, so GOTCHA 2's garbling risk is lower — but they are unverified against the PDF pages and should be checked before anything depends on them numerically.
  - +2 ux-spec checks (solo withholds session XP; group still banks it); 3 `REFERENCE.solo` entries rewritten from the quotes rather than from my design.

- **Oracle & Eye verified against the rulebooks (2026-08-24, SW v122, harness 224/224):** second round of NotebookLM answers. Two safe fixes made, one numeric discrepancy deliberately **left alone**.
  - ✅ **Fellowship Focus suppression confirmed correct.** *"When playing as a solo Player-hero, you are not a member of a Company and therefore do not have a Fellowship Focus … ignore this aspect of character creation."* The wiring added in the reachability pass is exactly RAW; now spec-pinned in both directions.
  - **Telling Table guidance added** (was absent): ask **closed** questions about the world; **phrase them so “yes” is the outcome your hero would prefer**; don't ask when the answer is already obvious from the story; open-ended questions belong to the **Lore Table**. Added to the Oracle card and rewritten into the glossary entry.
  - **Revelation Episode now offers the reset.** RAW: *"Once the Revelation episode is resolved, the Eye Awareness level resets and the tally begins again."* The old alert merely told the player to go and press the ↺ button; it now offers **"↺ Resolved — reset the Eye"** inline.
  - ⚠️ **Hunt thresholds: discrepancy logged, NOT acted on** — see open item 1b. NotebookLM gives five regions (Free 20 · Border 18 · Wild 16 · Shadow 14 · Dark 12) against the app's three (`{border:18, wild:16, dark:14}`). The *structural* claim (five regions, including Free and Shadow lands) is qualitative and probably right; the *numbers* form a perfect arithmetic ladder, which is precisely the pattern GOTCHA 2 records NotebookLM inventing. **Verify against the rendered PDF page before editing `HUNT_THRESHOLDS`.**
  - +3 ux-spec checks (Focus suppression both ways; Telling Table phrasing rule present and resolvable).

- **Eye Awareness & the Oracle, verified (2026-08-24, SW v123, harness 229/229):** round 3 of rulebook answers found **two real rules bugs** that had been live since the Strider Mode work.
  - 🐞 **A Rune was raising Eye Awareness.** RAW: *"Raise the Eye Awareness by 1 point whenever a die roll made by a player outside of combat produces an [Eye] icon"* — an **Eye only**. A Rune instead offers the **Fortune table**, whose Rune result *lowers* EA by 1. The old hook added +1 on either, so a Rune both invented a gain and cancelled the loss it should have produced — solo campaigns were running measurably darker than the rules intend. Now Eye-only.
  - 🐞 **Fortune / Ill-Fortune were gated on success/failure.** The app offered Fortune only on *Rune + success* and Ill-Fortune only on *Eye + failure*. RAW puts **no such condition** — the tables trigger on the Feat die alone — and frames the roll as optional, *"for worthy challenges and key actions"*. Both now offer on any Rune or Eye, labelled optional.
  - **Missing rule added:** EA *"is fully reset to its starting value at the beginning of each Adventuring phase"*. Completing a Fellowship Phase now resets it (`fpComplete`). Previously only the post-Revelation reset existed.
  - ✅ **`XP_MILESTONES` verified** — all ten entries match Strider Mode p.15 in the book's own order. The earlier worry that this list was invented (it carried no citation, unlike the Moria list) was unfounded; citation added. **Point values still want a PDF check.**
  - **Lore Table guidance added** (it had none, exactly as the Telling Table didn't): roll **one** column for a word or two–three for a phrase; read it literally or abstractly; **if it doesn't fit, roll again or check adjacent rows**.
  - ✅ **Already correct, no change:** the `cancel` Special Success spend was already labelled *"Co-op only"*, which matches RAW.
  - **Known remaining gaps** (documented, not implemented): the variable severity of magical displays (NPC Wizard 1/2/3 by spell scale), and Moria's *Eyes in the Dark* event doubling all subsequent EA gains.
  - +5 ux-spec checks pinning: Eye raises by 1, Rune does not, both offer their table regardless of outcome, the offer is marked optional, and the Lore Table explains re-rolls.

- **Hunt thresholds corrected — five regions (2026-08-24, SW v124, harness 231/231):** open item 1b closed. The owner confirmed the round-2 answer, so `HUNT_THRESHOLDS` goes from three regions to five: **Free 20 · Border 18 · Wild 16 · Shadow 14 · Dark 12**. The old table's `dark: 14` was really the *Shadow lands* value, and Free/Shadow were missing entirely; the region picker now offers all five with their full names.
  - ⚠️ **Derived consequence, deliberately surfaced rather than shipped silently: Moria got harder.** `toggleMoriaMode()` sets `huntRegion='dark'`, so Moria's Hunt threshold moved **14 → 12** — Revelation Episodes now fire two points sooner in every Moria campaign. That is the faithful reading (Moria is described as a Dark Land, and the old 14 came from the wrong table), but **if the Moria supplement prints its own threshold it wins** — a one-line change if so. Worth asking the notebook.
  - **A spec had pinned the wrong value.** `gm.js` asserted `Hunt 14` for dark; that check was encoding the bug, so it now asserts 12. New ux checks pin the whole five-region table and the picker's options, so a future edit cannot quietly drop a region again.
  - **Note on GOTCHA 2:** this is the first numeric table changed on a *human* confirmation rather than a PDF render. The ladder's suspiciously regular 20/18/16/14/12 shape is exactly what that gotcha warns about, so the confirmation is recorded here as the authority. If the printed page ever disagrees, this entry is the thing to revisit.

- **The adventure loop — when each subsystem fires (2026-08-24, SW v125, harness 237/237):** the saga card was still not enough. The owner: *"After creating character, don't know where journey comes in, when to explore location, when combat, when fellowship."* The saga card supplies the **campaign** arc (begin → sessions → end); what was missing is the layer under it — **the shape of one adventure**, which is what actually tells you when to open which tab.
  - **New `#adventure-loop` card** (Character tab, shown only once a saga has begun) with `char.saga.step` + migration. Five steps, tappable, remembered: **1 At a Safe Haven** (learn why you must leave) → **2 On the road** (the *Journey* subsystem — marching tests and events, not narration) → **3 At the place you travelled to** (the adventure proper; skill rolls, and a *Council* when the stakes are social) → **4 The road home** → **5 Fellowship Phase** (after two or three sessions). A full circuit increments `saga.adventures`.
  - Each step states **what it is**, **what to do now**, and carries a button that opens the tab it needs — so "where does the journey come in" is answered by the card itself rather than by prose elsewhere.
  - **The key misconception is stated outright:** *"Combat is not a step."* A fight can interrupt any step; run it on the Combat tab and return. Councils work the same way. That single sentence is probably the most load-bearing text in the feature — the question implied that fights and councils were being sought as *phases*, which they are not.
  - **Lesson repeated from the saga pass:** the fix was structural, not more explanation. Sixteen passes of prose did not answer "when does journey come in"; a five-step tracker with the current position highlighted does.
  - +6 ux-spec checks (five steps, each pointing at a real tab; hidden before a saga starts; advancing and remembering; naming its subsystem; counting an adventure; and the combat-is-not-a-step line).

- **▶ PLAY MODE — the app runs the session (2026-08-24, SW v126, harness 247/247):** third round of *"I still don't know how to start playing"*. The diagnosis that finally mattered: **the saga card and the adventure loop both POINTED at tabs** — *"open the Journey tab and fill it in"* — which is still homework. The owner asked to *"just directly play straight from the app"*, so this drives instead of pointing.
  - **New `▶ Play` tab, placed FIRST**, so the app opens on the game rather than on a character sheet. One screen: what is happening → a handful of concrete choices → tap one → the app performs the rolls, narrates the outcome in plain language, and moves to the next moment.
  - **It runs the subsystems itself.** `playSetOut()` asks where and how far and builds `char.journey`; `playTravel()` rolls TRAVEL via `_doInlineRoll`, advances hexes, adds Fatigue on a failure; `playAttempt()` offers **plain-language actions** ("🔍 Search the place", "🤫 Move unseen") mapped to skills so the player never picks a *skill*; `playAsk()` runs the Telling Table; `playRest()` drives a Prolonged Rest. A whole session needs no other tab.
  - **Everything narrated is written to the Chronicle** (`pushBlock(..., 'play')`), so the journal fills itself for someone who never opens that tab.
  - **Empty states are entry points, not dead ends:** no hero → offers pre-gens or Build; no saga → offers `sagaBegin()`; ended → offers `sagaReopen()`.
  - **Bug caught in testing:** `_doInlineRoll` returns `total: null` on a Gandalf rune, which rendered as *"roll — vs 17"*. Now reads *"a Gandalf rune — automatic success"*; spec-pinned so the em-dash form can't come back.
  - **The lesson, now three times over:** explanatory text does not teach a procedure, and neither does a tracker that tells you which tab to open. The thing that works is the app doing the step and reporting what happened. Sixteen passes of prose, one saga card and one loop tracker all failed on the same question.
  - +7 ux-spec checks driving a full loop through Play alone (begin → set out → travel → arrive → attempt → home → next adventure), plus the Chronicle write-through and the rune-display fix.

- **FIRST ACTUAL PLAYTHROUGH — a session played end to end (2026-09-12, no code change, nothing fixed):** every audit so far asked a *structural* question (is this control reachable, is this rule implemented, does this function exist) and all of them stay green on an app nobody can play a session with. A new **`solo-rpg-playtester` agent** (`.claude/agents/solo-rpg-playtester.md`, portable to other projects) asks the one they cannot: **can a person sit down, start a session, play it through, and finish — pressing only what the app offers?** It builds its own Playwright driver over `tests/browser.js`+`serve.js`, finds controls **by visible label** (never by selector — a control a player can't identify by reading the screen is itself a finding), and keeps state on disk between beats.
  - **Verdict: yes — the session played, start to finish**, with **zero genuine console errors**: manual Build → saga begin → session 1 → journey → location → combat → endeavour → council → Fellowship Phase → Moria battle → session 2 → saga end. Narrative + findings: **`docs/playtest-2026-09-12.md`** (17pp).
  - **16 findings, none of which any existing spec could have caught.** The worst are *routes between two things that both already work*:
    - 🐞 **▶ Play dead-ends after a Journey-tab arrival** — Arrive clears `char.journey.active`, so `🏁 We have arrived` never renders and step 3 is unreachable from the tab that claims *"You never need the other tabs."* (`src/04-render.js:3524`)
    - 🐞 **Strider Mode silently deletes the Prowess virtue's −1 TN** — TN is recomputed as a flat `18 − Rating`, ignoring `char.prowessAttr`, while the sheet still lists the virtue. **Two rolls in-session were made against the wrong TN.** (`src/01-core.js:198`)
    - 🐞 **FP wizard step 3 says "(Phase state is preserved.)" — it isn't.** `openFPWizard()` rebuilds `fpState` from scratch, so following the on-screen instruction resets phase type + recovery and lets **Recovery be applied twice in one phase** (observed −1 then −2). (`src/04-render.js:353`)
    - 🐞 **You cannot choose to end your saga.** `sagaEnd()` has one call site, gated on `sagaEndSignals()`; the Shadow signal **self-erases** (Bout of Madness fires on the same threshold and zeroes Shadow), leaving only Valour/Wisdom ≥ 5. (`src/04-render.js:3388`)
    - 🐞 **Journey events say "▶ Roll Awareness" and supply no control** — `#journey-log-card` has zero buttons. (`src/04-render.js:2502`) · **Clash successes become unspendable after a reload** — `renderClashSpend()` is never called from `renderBattle()`. (`src/04-render.js:1691`) · **Session recap shows the three *oldest* timeline entries labelled "Last time"** — `timeline` is newest-first, code does `.slice(-3).reverse()`. (`src/04-render.js:3276`) · **🌑 Shadow Tests / Combat Tasks / Special Success are unreachable on a fresh Dice tab** — nested inside `#roll-result`, which is `display:none` until an unrelated roll. (`character-tracker.html:1433`) · **Play and Journey measure the same journey in incompatible units** (2/4/7 hexes vs `3+icons` per test — one test consumed a whole journey). · **Dying is never explained anywhere** — one subordinate clause inside the *Endurance* glossary entry; the Play tab shows `Endurance 0/26` and the same five choices as at full health.
  - **The lesson, and it is the same one as the last three passes:** 70 (?) hints, 14 tab intros, a saga card, a loop tracker and a Play tab all pass their specs, and a player still hit **11 places where the app described something it gave no way to do.** Reachability walks *code → user* and coverage walks *document → code*; **neither walks the session**, which is where these live. `docs/reachability-audit.prompt.md` + `docs/coverage-audit.prompt.md` now have a third sibling in the agent itself.
  - **Nothing was fixed** — the run was report-only by instruction. The 16 findings are the live backlog; see open item 8.

- **ALL 16 PLAYTEST FINDINGS FIXED (2026-09-12, SW v127, harness 265/265):** open item 8 closed. Every fix is pinned by a check **proven to fail with the fix reverted** — the discipline earned its keep immediately: one check passed on a reverted fix because it matched the *prose* mentioning the button rather than the button (now GOTCHA 18).
  - **The one correctness bug. 🐞 Prowess −1 TN.** Nine separate sites wrote `(striderMode ? 18 : 20) - rating`, and the virtue's −1 was a one-off decrement to the stored TN — so *any* recompute (mode toggle, attribute set, lifepath) erased it while the sheet still listed the virtue. Now **one derivation**: `attrTN()` / `recomputeAttrTNs()` over a tracked `char.tnAdjust` (+ migration back-filling from `prowessAttr`, which is consistent because the stored TN already had the −1 baked in). The Lifepath Major-Event `TN+1`s moved into the same field. See GOTCHA 19.
  - **The routes that didn't exist** — the class that hid from every static check, because both ends already worked:
    - **▶ Play ↔ Journey.** Arriving on the Journey tab clears `journey.active`, so Play offered only "Set out" and step 3 was unreachable. Play now offers **🏁 We have arrived** for a journey finished elsewhere, `playArrive()` runs the *real* arrival (mount Vigour, arrival TRAVEL roll, lingering Fatigue) instead of a lesser copy, and **"the road home" gained `playSetOutHome()`** — it used to answer *"You are not travelling yet"* every single time.
    - **Journey events now bring their own roll.** *"▶ Roll Awareness"* with no control anywhere is fixed by `pendingEventRoll` + `#j-event-roll-row` + `JOURNEY_EVENT_ROLL_EFFECT`, which applies what the event promised (−1 day on a Short Cut, +1 Hope on a Joyful Sight, and so on) for all 7 Core + 6 Moria events. A Noteworthy Encounter correctly arms nothing — it is an extended scene, not one roll.
    - **One journey, one unit.** `playTravel()` now advances through `applyMarchingTestResult()` (the Journey tab's own `3 + icons`) instead of a second, incompatible 2-per-press rule; set-out distances are 4 / 9 / 18 hexes. The two surfaces are now one subsystem, not two that disagree.
    - **Shadow Tests / Combat Tasks** were children of `#roll-result` (`display:none` until a roll) — 0×0 on a freshly loaded Dice tab. Moved out as siblings. *(The Special Success panel stays inside: it spends icons from the roll just made, so it is genuinely roll-scoped.)*
    - **You can always end your saga.** `sagaEnd()` was gated on `sagaEndSignals()`, whose Shadow signal **self-erases** (the Bout of Madness fires on the same threshold and zeroes Shadow). The control is now always present; the app's *opinion* ("This could be an ending") still waits until one is earned.
  - **Two things the app said that weren't true.** The FP wizard's *"(Phase state is preserved.)"* — `openFPWizard()` rebuilt `fpState` every time, and because `recoveryApplied` reset after Hope and Shadow had already moved, **Recovery could be applied twice in one phase**. State now lives on `char.fpWizardState` between openings, the guard is durable, and step 3 spends XP in place (`fpSpendFromWizard`) instead of pointing at another tab. And the **Rangers' printed weakness** — *"only ½ Heart Hope recovered during Fellowship Phase (not Yule)"* — is now applied (`fpHopeRecovery()`), Yule exempt.
  - **Also:** *"Last time"* recapped the three **oldest** events (`timeline` is newest-first; `slice(-3).reverse()` → `slice(0,3)`) · pending **Clash successes** survive a re-render (`renderBattle` calls `renderClashSpend`) · **Dying** has a real glossary entry, a `data-hint`, a red banner on the Play tab and recovery promoted to the first choice · the **Hunt hint** lists all five regions (the Band preview's hard-coded `(14 +0)` beside a live `12` is gone; the Moria dialog says Hunt 12 and no longer claims Band/Battles "arrive in later phases") · **Moria now gives the Balin and the First Hall it promises**, stashing and restoring the hero's own Patron/Haven/region on the way out (it used to leave ex-Moria heroes on a Dark Land's Hunt 12 forever) · Council and Endeavour **modifier controls moved inside the phase that reads them**, with their captions, and the orphaned empty caption boxes deleted · a starting **Band of six has six different dwarves** · the Journey region picker offers all five regions · the hoard roller defaults to a party of 1 in solo · **"Begin the next adventure" now asks for the new reason it promises** · residual *"LM approval"* / *"the Company"* wording replaced or given `descSolo`.
  - **A self-inflicted lesson worth keeping:** mid-session I ran `git checkout -- <file>` to undo a deliberate revert-probe, on a tree where the fixes themselves were **uncommitted** — and destroyed two files' work. **Commit before probing.** The agent's own trap list says "snapshot state before pressing anything destructive"; it applies to the person testing the fix, not just the agent.

- **PLAYTEST RUN 2 — wider, and it found a regression I shipped (2026-09-12, no code change, nothing fixed):** the agent replayed the app against run 1's own *"Not covered"* list — **pregen hero** this time, not a hand build — and re-tested all 16 run-1 fixes **in play** rather than by reading the diff. ~300 driver invocations, **0 console errors**, and the session again played start to finish. Narrative + findings: **`docs/playtest-2-2026-09-12.md`** (16pp). It reused and extended run 1's driver rather than rewriting it (seeded PRNG, `snap`/`restore`/`hash`/`check`, and a bypass for the harness's tutorial suppression).
  - 🔴 **The finding that matters most is mine.** **A Fellowship Phase can now only be run once per character, ever.** `fpComplete()` nulls `char.fpWizardState` (`src/04-render.js:700`) and then calls `fpClose()` (`:809`) → `fpPersist()` → which writes it straight back carrying `recoveryApplied: true`. Every later phase refuses Spiritual Recovery permanently, and the refusal text I wrote (*"Cancel and reopen to redo"*) is false. `openFPWizard(forceNew)` exists and **has no caller**. This is a **regression introduced by the run-1 fix for finding 6** — the durable guard worked exactly as designed and the teardown didn't. Reproduced from a virgin hero.
  - 🔴 **FP step 3 opens Spend XP *behind* the wizard** — both overlays are `z-index:200` and the wizard is later in the DOM, so 0 of 19 buttons are hit-testable while the card says *"the wizard stays open underneath"*. Closing the wizard clears `fpModeActive`, so the only usable XP route is the one that announces *"no caps enforced"* — Valour **and** Wisdom raised in one phase. The run-1 fix for finding 6's step-3 half is therefore **cosmetically present and functionally unusable**.
  - 🔴 **Clash spend regressed.** Run-1 finding 7 was fixed by calling `renderClashSpend()` from `renderBattle()` — it does run, and emits 706 characters of markup into `#b-spend`, which sits inside `#b-clash-result` (`display:none`). Same container-visibility trap as run-1 finding 10, which was fixed by moving elements *out* of a hidden parent; here I only fixed the renderer. **Calling a renderer is not the same as making its output visible.**
  - **Other real ones:** a Valour/Wisdom rank-up not spent in the moment is **lost forever** (`openNewReward`/`openNewVirtue` have one call site each; 16 AP spent, nothing received) · **Journey Events are unreachable from ▶ Play** (`playTravel` sets `nextEventHex`, `renderPlay` never reads it) · a journey event's **failure branch applies nothing** while the Fatigue from the same event auto-applies — so the run-1 fix covered success only · **every pregen's Patron matches no table** (`"…Dírhael"` vs key `'…Dirhael'`; Moria writes `'Balin'` against `'Balin, son of Fundin'`), which breaks Patron Quest, `sagaBegin` seeding and the FP "Meet Patron" undertaking · **Lifepath Parry ±1 is erased on the next render** — GOTCHA 19 moved Lifepath *TN* into `tnAdjust` and left *Parry* as a one-off write to a derived value · an **abandoned Bout of Madness locks permanent Despair** (`_boutPrompted` latches, Harden Will is `disabled` at exactly that threshold and silent about it, FP recovery is blocked by the regression above) · **Eye Awareness only rises on Dice-tab rolls** — an 👁 in a Council, Journey, Endeavour, Battle or Play roll does nothing, and Fortune/Ill-Fortune are never offered there · ▶ Play **corrupts apostrophes in the Chronicle** (`playSay` strips tags but not entities → `steading&#39;s`) · Skirmish/Rearward print *"melee cannot attack"* and then allow it at full effect · "Fly, You Fools!" sends the escape roll to the Dice tab, where foe Parry is not applied (**TN 17** on Combat, **TN 12** on Dice for the same swing) · the **hoard party-size fix never fires** (the input ships `value="4"`, so `|| (isSolo()?1:4)` is unreachable) · **2h grip is unavailable on all 13 pregens** (`inj1h/inj2h` are only set by `pickWeapon`) · plus 13 minors.
  - **Run-1 verification: 12 held cleanly, 3 held with caveats, 1 regressed, 1 cosmetic-only.** Full table in the doc.
  - **The lesson, and it is a new one:** run 1 proved a green harness is not evidence the game is playable (GOTCHA 16). Run 2 proves **a fix is not evidence of a fixed behaviour** — three of the sixteen were verified by a spec that tested the mechanism I changed rather than the outcome a player gets. A check that asserts `renderClashSpend` ran, or that `fpWizardState` is nulled, passes over all three of these. See GOTCHA 20.
  - **Coverage delta:** newly reached — the whole tutorial (offer, sandbox persistence, boot recovery, interactive auto-advance, Keep *and* Discard), pregen load-and-play, Random Lifepath, Yule + Heal Scars, Treasure Index, dormant-quality unlock, cursed item + Shadow Taint through an FP, every Moria generator, Desperate Stand, Hero-or-Band, Kingly Gift, Moria FP + Interruption, roster switch/rename/duplicate/delete, share link + QR gate + hash import, export single + bulk, Restore Points, Undo, print, themes/text-size/compact/Table Mode, the GM tab incl. cross-hero hand-out and Group Shadow Test, Spend XP + both rank-ups + Cultural Virtues at Wisdom 2, Bout of Madness, Harden Will, rest/day-tracker/First Aid, SoL auto-promote, the Chronicle's generator/oracle/combat groups/both exports, and multi-foe combat to a slain foe. Still unreached — cloud/campaign (SDK stubbed), Opening Volley and Pierce against an Encounter foe (Dice-tab only), the Moria Archfoe Duel, Restore-Points *restore*, Import Character, print render, and the 4-Flaws retirement (blocked by the Bout lock).
  - **Nothing was fixed** — report-only by instruction. The backlog is open item 9.

- **ALL 17 RUN-2 FINDINGS + THE 13 MINORS FIXED (2026-09-13, SW v128, harness 280/280):** open item 9 closed. Every fix carries a check **proven to fail with the fix reverted**, and — per GOTCHA 20, which exists because three run-1 fixes passed specs while staying broken in play — each check does what a player does: presses the control, reads the number, runs the flow twice.
  - **The regression I shipped is gone.** `fpComplete()` nulled `char.fpWizardState` and `fpClose()` → `fpPersist()` wrote it straight back with `recoveryApplied: true`, so no hero could ever take a second Fellowship Phase. Fixed with the rest of the first batch (findings 1–8, 14, 15, 17): FP step 3's Spend XP now opens **above** the wizard (`z-index: 240`), an unspent Valour/Wisdom rank-up is **owed rather than lost** (`pendingRewards`/`pendingVirtues` + standing claim buttons in `#owed-picks`), Clash spend moved **out of** the `display:none` `#b-clash-result`, Journey Events are reachable **from ▶ Play**, a journey event's **failure branch applies what it announced**, every pregen's **Patron resolves** (`patronKey()` folds accents and maps `Balin` → `Balin, son of Fundin`), Lifepath **Parry ±1 lives in `char.parryAdjust`** like the TNs do (GOTCHA 19 generalised), the hoard roller's solo default is reachable (the input's own `value="4"` made the fallback dead code), **2h grip back-fills on all 13 pregens** (`_ensureGripData`), and the **Eye rises on any solo roll outside combat**, not only Dice-tab ones (`_soloEyeFromRoll` + `_suspendInlineEye` around combat).
  - **The Bout of Madness no longer locks a hero in permanent Despair.** The prompt fires on a timer; losing it latched `_boutPrompted` forever, and **Harden Will — the one control that resets it — was rendered `disabled` at exactly that threshold**, so pressing it produced nothing and said nothing. Now `char.boutDue` keeps the bout claimable until it is actually resolved, a standing *"🌑 Face the Bout of Madness"* control sits on the Character tab, and Harden Will is never `disabled` — it stays hit-testable so its own guard can explain itself. *The lesson: a disabled button is a refusal with no route, which is the same failure as a dead one.*
  - **Two places the app stated a rule and then broke it.** Skirmish/Rearward printed *"melee cannot attack"* and rolled at full effect — it now blocks and offers the stance change. **"Fly, You Fools!"** sent the escape roll to the Dice tab, where the foe's Parry is not in the TN (**12 there, 17 on Combat for the same swing**); the escape is now rolled in place against the engaged foe, and the typed `1`/`2` prompt that ignored anything else is gone.
  - **Two gaps in the record.** ▶ Play stripped tags but not entities, so the Chronicle filled with `steading&#39;s` and the stripped `<br>` welded sentences together (`_playPlainText`). And the **Campaign Timeline recorded one entry for a whole adventure**: rank-ups bought through Spend XP never touch `adj()` and `hardenWill` writes `char.scars` directly, so neither was ever seen. Logged at the funnels now — every rank-up, Harden Will, journeys, councils, endeavours, fights, each adventure.
  - **Moria caught up with run 1**: the Revelation dialog offers the Eye reset inline, `moriaFP()` routes Hope through `fpHopeRecovery()` so the **Rangers' halving applies** there too, and a Moria phase resets Eye Awareness for the new Adventuring phase.
  - **All 13 minors**: the PE card reads the real budget (it said 10 while solo grants 15) · the end-session confirm no longer promises XP a Milestone/solo hero never gets · the Oracle's Fortune/Ill-Fortune cards stopped gating themselves on success/failure (that condition was removed from the rules handling in August) · ▶ Play can **choose the Telling Table's odds band** instead of always Middling · the Play vitals show the **auto-WEARY** the Character tab is pulsing · `Spend SP`/`Spend AP` and `Roll backstory`/`Roll event` are told apart by their labels · **days pass on the road** (`advanceDays` — a nine-day march left "Day 1" and never ticked injury days) · fell abilities that name a round trigger are **put in front of the player** each round · the **share link carries the hero, not the play record** (15,056 chars → under the QR gate) · `possessive()` handles `Duinhir 'Eaglenose'` · Restore Points say what each snapshot **is**, with its vitals, instead of eight rows reading `load` · Table Mode names a nameless hero by what it is rather than `?` · the **Play loop opens a Chronicle scene** at each break · both tutorial *"Try it"* steps are interactive · the practice-hero choice offers **Keep / Discard**, not OK/Cancel.
  - **+15 ux checks (174 → 189, harness 265 → 280)**, each probed by reverting its fix and confirming the check goes red. Two were rewritten mid-probe because the first version passed on a reverted fix: the Bout check originally *set* `boutDue` by hand (so it asserted the renderer, not the lock) and now loses the prompt the way a player does; the Rearward check originally depended on the dice (a miss looked like a block) and now forces every roll that happens to land, so "the foe took no damage" can only mean no attack was made.

- **PLAYTEST RUN 3 — the fixes hold, and the seams between them do not (2026-09-13, no code change, nothing fixed):** the agent replayed the app against run 2's *"still unreached"* list, on a pregen scholar, and re-tested all 17 run-2 fixes **in play**. **469 driver invocations, 0 page errors**, and the session again played start to finish — pregen → Strider → saga from the Patron → forced-march mounted journey with a peril → Council + Skill Endeavour → a Barrow-wight → hoard + cursed treasure → **four** Fellowship Phases including Yule → a second adventure → five Bouts of Madness → the **4-Flaws succumbing** run 2 was locked out of → saga closed. Narrative + findings: **`docs/playtest-3-2026-09-13.md`** (and a rendered PDF beside it).
  - **Run-2 verification: 12 held cleanly · 4 held with caveats · 1 minor not met · 0 regressed.** The regression I shipped is genuinely gone — four Fellowship Phases on one hero, across reloads. Spend XP is hit-testable above the wizard (19/19 buttons), Clash spend survives a reload visibly, every Patron resolves, Lifepath Parry survives a re-render, the Bout no longer locks, Skirmish blocks melee, the Chronicle reads as prose, the Timeline records the adventure, Moria's three all landed.
  - **The new lesson: a fix applied at one call site is not a fixed behaviour.** Three of the five worst findings are a run-2 fix that reached one branch of two:
    - 🐞 **The road home has no Journey Events.** `_playEventDue()` was added to `_playStepChoices()`'s `case 'journey'` and not to `case 'home'` (`src/04-render.js:3997`) — the return leg schedules events exactly as the outbound leg does, and Play offered *"🥾 Travel onward"* past two of them.
    - 🐞 **A Reward you earned is destroyed if you cancel the "Apply to…" step.** `pickNewReward` decrements `pendingRewards` **before** `promptApplyReward` (`src/05-combat-build.js:2510`). Run-2 finding 3 made the rank-up survive a dismissed *picker*; it does not survive a dismissed *apply*. 16 AP spent, two Rewards gone, nothing owed.
    - 🐞 **▶ Play cannot see a journey started on the Journey tab, and silently destroys it.** Play read *"AT YOUR HOME — You are somewhere safe"* during a live 8-hex mounted forced march, and **🥾 Set out** overwrote it with no confirmation (`:3951`, `:4165`). Run-1 finding 1 fixed the *arrival* hand-off; the *departure* one was never there.
  - **Two more that stop play:** the game's worst wound, `Grievous Injury 👁 — Unconscious & Dying`, ships `days: 0`, so **🩹 Tend the wound** — which Play promotes to the top while you are Wounded — answers *"No Severe Injury day-count to reduce"* forever, and four Fellowship Phases did not clear it. And a number the Bout of Madness did not offer **silently wipes all your Shadow and gives no Flaw** (`src/04-render.js:231`) — no message, the bout consumed, and the mistake is strictly to the player's advantage.
  - **19 substantive findings + a 14-item minor bundle** (High 5 · Medium 9 · Low 5). One run-2 minor is **not met**: the share link is **6,092** characters against the 1,200-char QR gate — trimming the play logs helped and was not enough.
  - **Coverage delta:** newly reached — the **4-Flaws retirement**, Restore-Points *restore*, Import Character, the print render, Opening Volley and Pierce against an Encounter foe, the Moria Archfoe Duel, Skill Endeavour at all three Risk levels incl. Disaster, Council Success-with-Woe, peril events, forced march, mounted travel + Vigour, the Treasure Index, Raise an Heir, Write a Song, Ponder Maps, Strengthen Fellowship expiring, the Band's tests, Desperate Stand, and the GM Group Shadow Test across several heroes. Cloud/campaign remains stubbed by design.
  - **Nothing was fixed** — report-only by instruction. The backlog is open item 10.

- **ALL 19 RUN-3 FINDINGS + THE 14 MINORS FIXED (2026-09-14, SW v129, harness 298/298):** open item 10 closed. **+18 ux checks (189 → 207)**, each probed by reverting its fix and watching it go red.
  - **The three that mattered were one fix applied to one branch of two, so all three are now structurally impossible to repeat:**
    - **The road home has its Journey Events.** The outbound and homeward legs no longer keep separate copies of the travel choices — **`_playRoadChoices(C, homeward)` serves both**, so neither can gain a feature the other lacks. Arriving home also *ends* the journey (`playArriveHome`), instead of leaving `journey.active` true for the rest of the campaign.
    - **A Reward survives a cancelled "Apply to…".** `pendingRewards` is spent in `confirmApplyReward`, at the moment the Reward is received — not in `pickNewReward`, before the step that can still be abandoned.
    - **▶ Play sees a journey started on the Journey tab** (the haven says so and offers *"Back to the road"*), and **Set out never overwrites a live journey silently** — it names what is under way and asks.
  - **The worst wound in the game does something.** `Grievous Injury 👁` applies its own printed effect (Unconscious and Dying → Endurance 0); a **Moderate** injury closes on a Prolonged Rest, exactly as its text says; and the First Aid row shows for **every** wound with a **"✔ The wound has passed"** control, so no injury is a dead end. Its refusal now names the route instead of saying "no day-count" forever.
  - **The Bout of Madness picks from buttons and reports the outcome.** It asked you to type a digit, numbered the Flaws by their *original* index, and silently wiped all Shadow while granting no Flaw for any number it had not offered — the strictly better outcome, quietly handed to anyone who mistyped.
  - **Also:** a Foolish **Disaster** stays on screen and announces itself (the renderer owns visibility, so the `render()` on the next line can no longer hide it) · **Valour is capped against itself**, not only against Wisdom · the **Rearward escape exists** (*"Slip away now"* disengages the foes) · **Opening Volley** doubles the shield Parry where the foe actually shoots, and only against ranged attacks · **Pierce works in the Encounter**, the surface the app tells you to fight on · **undertakings persist on every toggle**, as step 3 promises · **Harry the Archfoe** banks a real pool and the **Duel puts the Archfoe into the Encounter** carrying it · ▶ Play **notices a retired hero** instead of offering it a fight · **Fortune / Ill-Fortune are offered on any solo roll**, as the Oracle tab claims (`fortuneTableRoll` is shared, and the offer gates on what is on screen rather than a flag that a force-hidden dialog could wedge forever) · journey events reach the feed and the Chronicle **as prose** · the **printed sheet is an ALLOW-list** — it had enumerated the tabs that existed when it was written, so ▶ Play and the whole GM Screen printed · the FP recovery refusal **stopped giving false advice**.
  - **The 14 minors:** `possessive()` keeps the 's on a name in quotes · the **share link is 363 characters** (was 6,092 against a 1,200 QR gate) — it carries the hero, not the session, with the 18 skills as one positional string that round-trips · **`heroLabel()`** names an unnamed hero everywhere (GM tab, Party View, Table Mode, Restore Points, which had been showing a raw hero id) · **Eye Awareness has a ceiling** at the Hunt threshold and prompts the Revelation there, having run to `👁 106/16` with the Revelation never returning · applying a Culture writes **an age, not the culture's range** · asides stay **out of the Chronicle** and an Oracle ask writes one entry rather than two · the **Orc-Band tallies** its foes and can join the Encounter · the return leg keeps the outbound **region** (and its capitalisation) · *"The road home to home"*, *"a Awareness roll"*, the Moria dialog's empty PE "from" value, a no-penalty event's *"the effect stands"*, and the Timeline's out-of-date blurb.
  - **Two spec repairs, both the same class as GOTCHA 22:** the journey-event check let the dice pick the event and passed about nine runs in ten; the undertaking check asserted an in-memory object that `fpState` aliases, so it passed while nothing was ever written — it now reads the saved slot back, the way a reload does.
...
| ~~1b~~ | ~~Verify Hunt thresholds~~ ✅ **confirmed by the owner 2026-08-24** | — | Five regions now live: Free 20 · Border 18 · Wild 16 · Shadow 14 · **Dark 12**. The app had three and labelled 14 "Dark" — that is really the *Shadow lands* value. ⚠️ **Consequence to watch: Moria.** `toggleMoriaMode` sets `huntRegion='dark'`, so Moria's threshold moved **14 → 12**, making every Moria campaign meaningfully harder. That follows from Moria being a Dark Land, but if the Moria supplement states its own threshold it overrides this — one-line fix in `HUNT_THRESHOLDS`/`toggleMoriaMode` if so. |
...
| ~~8~~ | ~~Fix the 16 playtest findings~~ ✅ **all 16 done 2026-09-12** | — | Harness **265/265**; every fix pinned by a check proven to fail when the fix is reverted. Details in the dashboard entry above; the session that found them is `docs/playtest-2026-09-12.md`. The remaining work from that report is not a fix but a habit: **re-run the playtester after anything that touches the session loop, lifecycle or oracle surfaces** (GOTCHA 16).

| ~~9~~ | ~~Fix the run-2 playtest findings~~ ✅ **all 17 + 13 minors done 2026-09-13** | — | Harness **280/280**; every fix pinned by a check **proven to fail when the fix is reverted**. The order that mattered, kept as the record of what stopped play: **(a)** the Fellowship Phase one-shot regression (`fpComplete` nulled `fpWizardState`, then `fpClose`→`fpPersist` restored it, so no hero could take a second phase); **(b)** FP step 3's Spend XP opening behind the wizard; **(c)** a rank-up not spent in the moment lost forever; **(d)** Clash spend rendering into a hidden container; **(e)** Journey Events unreachable from ▶ Play; **(f)** pregen Patron keys matching no table; **(g)** Eye Awareness rising only on Dice-tab rolls. Details in the dashboard entry above; the session that found them is `docs/playtest-2-2026-09-12.md`. **Deploying is no longer blocked — but play it again first (GOTCHA 16).** |

| ~~10~~ | ~~Fix the run-3 playtest findings~~ ✅ **all 19 + 14 minors done 2026-09-14** | — | Harness **298/298**; every fix pinned by a check **proven to fail when the fix is reverted**. The three worst were one earlier fix applied to one branch of two, so the pairs are now single code paths (`_playRoadChoices` for both legs; the Reward claim spent on receipt; the print sheet an allow-list). Details in the dashboard entry above; the session that found them is `docs/playtest-3-2026-09-13.md`. **Deploying is no longer blocked — play it once more first (GOTCHA 16).** |

**Recommended order: 1 (deploy, after a fourth playtest) → 2 (table test). 5 and 7 only if a real need emerges. Items 3, 4, 6 ✅ done 2026-07-02; items 8, 9, 10 ✅ done 2026-09-12 / 09-13 / 09-14.** Every playtest backlog is closed. **Play it once more before deploying** (GOTCHA 16): runs 2 and 3 each found what the previous round's fixes broke or half-reached, and this round changed the session loop, the wound rules, combat and the share format. If run 4 finds only cosmetics, ship it.
...
14. **Onboarding is a tested invariant, not decoration** — every tab must keep a `.tab-intro`, jargon gets `data-hint="<Term>"`, and a first-run prompt must never stamp `offered` before the user answers. **15 ux-spec checks** enforce this, including a **dead-hint guard** (every `data-hint` must resolve via `hintRow()` — `_attachHint` silently renders nothing for an unknown term, so a typo would otherwise vanish unnoticed). Adding a tab or a jargon control means adding its intro/hint too. *(→ Newcomer onboarding passes 1 & 2)*
15. **Adding a shipped file means three edits, not one** — the `<script>`/`<link>` tag in **both** shells, the `PRECACHE` list in `sw.js`, and a `CACHE_VERSION` bump. *(→ dev workflow)*
16. **A green harness is not evidence the game is playable** — that is the premise of the `solo-rpg-playtester` agent, and it was borne out the first time it ran: 247/247 green, and a played session still hit 11 places where the app described something it gave no way to do. Reachability walks *code → user*, coverage walks *document → code*; **neither walks the session**. Before shipping anything that touches the session loop, lifecycle or oracle surfaces, **play it**. *(→ First actual playthrough)*
17. **Find controls by their visible label when driving the UI, never by selector** — a control a player cannot identify by reading the screen is itself a finding, and a selector-based driver hides exactly that class of bug. Likewise a control that only *scrolls* to a panel is not the control that *acts*; they look identical in a transcript. *(→ the agent's own trap list)*
18. **A regression check must assert the CONTROL, not the prose around it** — the first version of the "Play recovers after a Journey-tab arrival" check matched the situation text, which happens to *mention* the button ("Tap **We have arrived**"), so it passed with the button deleted. It now queries `button[onclick="playArrive()"]`. Every check added for a playtest finding was proven to fail with its fix reverted; this one is why that step is not optional. *(→ Playtest fixes)*
19. **One derivation, one place: `attrTN` / `recomputeAttrTNs`** — an Attribute TN is `base − Rating + char.tnAdjust[attr]`, where base is 20 (18 in Strider Mode). The Prowess virtue's −1 and the Lifepath Major-Event +1s live in `tnAdjust`, NOT as one-off decrements to the stored TN — applying them directly is what let a Strider-Mode toggle silently erase Prowess while the sheet still listed it. Never write `(striderMode ? 18 : 20) - rating` at a new site. *(→ Playtest fixes, finding 3)*
20. **A fix is not evidence of a fixed behaviour — assert the player-visible OUTCOME, not the mechanism you changed.** Three run-1 fixes passed their specs and were still broken in play: the Clash-spend check asserted `renderClashSpend()` had emitted markup (it had — into a `display:none` parent); the FP-state check asserted `fpWizardState` was nulled by `fpComplete` (it was — and then written back by the `fpClose` on the next line); the hoard party-size fix added a `|| (isSolo()?1:4)` fallback the input's own `value="4"` makes unreachable. A check must do what a player does and look at what a player sees: **is the control hit-testable, did the number on screen change, can the flow be run twice.** *(→ Playtest run 2)*

21. **A `disabled` button is a refusal with no route — never disable the control that IS the way out.** At exactly the Bout-of-Madness threshold, **Harden Will** (the only control that resets `_boutPrompted`) was rendered `disabled`, so the click was swallowed and its two explanatory guards were unreachable. Combined with the Fellowship-Phase regression, a hero could be locked in permanent Despair with no rules route out. Dim a control that does not apply if you must, but keep it clickable so its guard can speak. *(→ run-2 fixes, finding 9)*
22. **A probe that depends on the dice proves nothing.** The first Rearward-melee check asserted the foe's Endurance was unchanged — which a plain miss also satisfies, so it passed on the reverted fix. It now stubs `_doInlineRoll` to always land, making "the foe took no damage" mean "no attack was made". Likewise the Bout check originally *set* `char.boutDue` by hand and so asserted the renderer rather than the lock; it now loses the prompt the way a player does. **When you prove a check fails on a revert, if it does not, the check is the bug.** *(→ run-2 fixes)*
23. **A fix applied at one call site is not a fixed behaviour — find every branch, then play them all.** Run 3's three worst findings are each a run-2 fix that reached one of two paths: `_playEventDue()` was added to `_playStepChoices()`'s `case 'journey'` and not `case 'home'`, so the road home lost its Journey Events; the rank-up rescue made a Reward survive a dismissed *picker* but not a dismissed *apply*, which still destroys it; the Play↔Journey hand-off was fixed for *arrival* and never existed for *departure*. Grep for the sibling case before calling any of these done, and drive both legs in play. *(→ Playtest run 3)*
24. **When two surfaces run the same subsystem, give them ONE code path — do not fix both.** Run 3's worst finding was Journey Events reaching the outbound leg and not the road home, because `_playStepChoices` held a copy of the travel choices per leg. Both legs now render from `_playRoadChoices(C, homeward)`, so neither can gain a feature the other lacks. Same shape elsewhere: the printed sheet is an **allow-list** (`.panel { display:none }` + the four sheet panels) rather than a list of tabs to hide, which is why adding ▶ Play and the GM tab silently added them to the character sheet. Prefer deleting the second copy to fixing it. *(→ Playtest run 3)*
25. **Gate a deferred offer on what is on screen, not on a flag.** The Fortune/Ill-Fortune offer used a `pending` boolean; a dialog hidden without resolving (the Escape handler can do that) left it `true` and the feature silently never fired again. It now checks for an open `.menu-overlay.show` and waits, so nothing can wedge it permanently. Any "only one of these at a time" latch needs the same question asked of it. *(→ Playtest run 3)*
...
- **`sw.js` `CACHE_VERSION`**: `tor2e-v129` (bump on every deploy). `PRECACHE` lists all 8 `src/*.js` + `vendor-qrcode.js` + `styles.css` + `firebase-config.js` + shells + PWA assets — **add any new file there**. SW strategy: HTML/navigations network-first; static assets cache-first; auto-activate.
- **Test harness**: `npm test` → 7 specs / **298 checks** (smoke 24, adversaries 11, ux 207, spillage 22, a11y 7, gm 20, **reachability 7**). A fresh clone needs `npm install` first (`playwright-core` is the only devDependency; `tests/browser.js` glob-resolves a cached Chromium, `CHROMIUM_BIN` overrides). Cloud paths are no-ops in tests (SDK blocked) — verify live features via preview against the real project.
...
- **localStorage keys**: a **multi-character roster** (added 2026-05-31):
  ...
  - *(the 2026-09-12 playtest-fix pass added no keys either — the two new fields live ON the character, so they export and sync: **`char.tnAdjust`** `{str,hrt,wit}` (standing Attribute-TN adjustments — Prowess −1, Lifepath Major-Event +1; see GOTCHA 19) and **`char.fpWizardState`** (the in-flight Fellowship Phase, so closing the wizard pauses instead of resetting). `char.journey.pendingEventRoll` and `char._moriaPrev` are likewise on the character. The 2026-09-13 run-2 pass added four more, all on the character: **`char.pendingRewards`/`char.pendingVirtues`** (a rank-up you were owed but have not chosen), **`char.parryAdjust`** (standing Parry adjustments — the Parry half of GOTCHA 19), and **`char.boutDue`** (a Bout of Madness triggered but never answered). The 2026-09-14 run-3 pass added three more, likewise on the character: **`char.flyPending`** (a Rearward fall-back whose escape has not been taken), **`char.injuryKind`** (`severe` / `moderate` / `grievous` — which rules end this wound) and **`char.injuryRested`**.)*
...
- [x] **Strider Mode subsystem** — full solo-play variant per *The One Ring – Strider Mode* supplement.
  ...
  **Strider Mode known gaps (audited 2026-05-27 — all 5 settled 2026-05-27):**
  ...
  5. ✅ **Fellowship Focus suppression** — `refreshStriderUI()` hides the Focus picker row (`#focus-row`) + the "Spend 1 Hope to support" hint (`#focus-hint`) when in a solo mode, and shows `#focus-strider-hint` in their place. **NB: this was documented as done but was never actually wired — the markup shipped, nothing referenced it, so the hint was permanently invisible and the picker never hid. Fixed 2026-08-20 and spec-pinned; see the reachability pass below.**
...
├── docs/playtest-2026-09-12.md         # the first played session, start to finish + its 16 findings
├── docs/playtest-2-2026-09-12.md       # run 2 — wider coverage; found a regression run 1's fix introduced
├── docs/playtest-3-2026-09-13.md(+.pdf) # run 3 — the fixes held; the seams between them did not
├── .claude/agents/solo-rpg-playtester.md  # the agent that played it (portable; builds its own UI driver)
