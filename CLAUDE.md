...
- **`sw.js` `CACHE_VERSION`**: `tor2e-v75` (bump on every deploy)
...
- [ ] **Promote `char.encounter` to a campaign-level shared encounter.**
...
- [~] **IN PROGRESS — local GM dashboard landed 2026-07-01.** `src/08-gm.js` (wired into both shells + `sw.js` PRECACHE, cache → **v89**) adds a **device-global** GM Screen behind a menu toggle (`🎲 Enable/Disable GM Screen`, key `tor2e-gm`), mirroring the solo-mode toggle pattern. When on, a hidden **`🎲 GM` nav tab** + `#panel-gm` appear (visibility owned by `refreshGmUI()`, called from `refreshStriderUI()`; rendered on tab-click via `bindTabs` and on every `refreshStriderUI`). **`renderGm()`** lists **every roster hero** (active starred, DYING flag) with live ❤/✦/🌑 and hand-out controls: **−1 / −3 End**, **+3 heal**, **Weary / Miserable / Wounded** toggles, **+Shadow**. All mutations route through **`gmMutateHero(id, mut)`** — the active hero edits live `char` + `saveCharacter()` + `render()`; other heroes edit their slot via `readSlot` → `localStorage[tor2e-char-<id>]` + `Sync.queuePush(id)` (mirrors to cloud when enabled). Shadow respects the `hopeMax − scars` cap. Guarded by a new **`gm` spec (8 checks)** — harness **83/83 green**. No character-schema change; no rules change (device-global infra only).
  ...
  - **Remaining for full P6 (deferred, depend on P4/P5/P7):** role-gating by `role === "loremaster"` (needs campaigns), peek-any-sheet read-only view, **drop adversaries into a *shared* encounter** (currently the note points players to the Combat-tab Bestiary; local only), the **broadcast feed** (needs RTDB), and porting the richer `tor2e-loremaster-main` GM tooling (group shadow tests, NPC ledger, Eye manager, Moria/journey/loot generators). The local dashboard is the standalone-usable slice; it "gains a LIVE shared party when cloud campaigns land" (per the module header).
