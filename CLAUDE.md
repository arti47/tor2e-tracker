...
## ⭐ STATUS DASHBOARD — read this first (updated 2026-09-13)
...
- **SW cache `tor2e-v128`** · git repo (origin = github.com/arti47/tor2e-tracker, branch main) · shells (`character-tracker.html` = `index.html`) in sync.
...
| **10** | **Fix the run-3 playtest findings** (2026-09-13) | **M** | **Blocks deploying.** 19 findings + 14 minors in `docs/playtest-3-2026-09-13.md`. Order by what stops play: **(a)** the road home has no Journey Events — the run-2 fix reached `case 'journey'` and not `case 'home'`; **(b)** a Reward is destroyed if you cancel the "Apply to…" step (`pendingRewards` is spent before the prompt); **(c)** ▶ Play cannot see a journey started on the Journey tab and silently overwrites it; **(d)** `Grievous Injury 👁` ships `days: 0`, so the wound never heals and its own control refuses forever; **(e)** an unoffered number at the Bout of Madness wipes all Shadow silently and grants no Flaw; **(f)** the share link is still 6,092 chars against the 1,200 QR gate. **Three of the five worst are one run-2 fix applied to one of two branches — when fixing, find every call site, then play both.** |

**Recommended order: 10 (run-3 fixes) → 1 (deploy) → 2 (table test). 5 and 7 only if a real need emerges. Items 3, 4, 6 ✅ done 2026-07-02, items 8 and 9 ✅ done 2026-09-12 / 2026-09-13.** Run 3 closed item 9's verification (12 of 17 held cleanly, 0 regressed) and opened item 10 — deploying now would ship an app whose road home has no events and whose worst wound cannot heal. **Play it again after item 10** (GOTCHA 16): each run so far has found what the last one's fixes broke or half-reached.
...
- **`sw.js` `CACHE_VERSION`**: `tor2e-v128` (bump on every deploy). `PRECACHE` lists all 8 `src/*.js` + `vendor-qrcode.js` + `styles.css` + `firebase-config.js` + shells + PWA assets — **add any new file there**. SW strategy: HTML/navigations network-first; static assets cache-first; auto-activate.
- **Test harness**: `npm test` → 7 specs / **280 checks** (smoke 24, adversaries 11, ux 189, spillage 22, a11y 7, gm 20, **reachability 7**). A fresh clone needs `npm install` first (`playwright-core` is the only devDependency; `tests/browser.js` glob-resolves a cached Chromium, `CHROMIUM_BIN` overrides). Cloud paths are no-ops in tests (SDK blocked) — verify live features via preview against the real project.
...
- **localStorage keys**: a **multi-character roster** (added 2026-05-31):
  ...
  - *(the 2026-09-12 playtest-fix pass added no keys either — the two new fields live ON the character, so they export and sync: **`char.tnAdjust`** `{str,hrt,wit}` (standing Attribute-TN adjustments — Prowess −1, Lifepath Major-Event +1; see GOTCHA 19) and **`char.fpWizardState`** (the in-flight Fellowship Phase, so closing the wizard pauses instead of resetting). `char.journey.pendingEventRoll` and `char._moriaPrev` are likewise on the character. The 2026-09-13 run-2 pass added four more, all on the character: **`char.pendingRewards`/`char.pendingVirtues`** (a rank-up you were owed but have not chosen), **`char.parryAdjust`** (standing Parry adjustments — the Parry half of GOTCHA 19), and **`char.boutDue`** (a Bout of Madness triggered but never answered).)*
