...
| Conditions | ~95% | |
| Skills | ~95% | |
| Shadow | ~95% | ✅ Shadow Tests + Harden Will + Scars-as-Shadow + 4-flaws-succumb retirement added. Still missing: Ill-Favoured all rolls when Shadow+Scars=Max Hope (currently only Bout prompt fires); auto-deduct of Shadow on a successful Shadow Test (player still adjusts the counter manually). |
| Journey | ~85% | ✅ Full Journey tab added. Roles, hex path, Marching Tests, Event Target + Journey Events table, terrain/season modifiers, Forced March, Mounted/Vigour, Arrival TRAVEL roll, lingering Fatigue → regular Fatigue, event log. Still missing: Perilous Locations (Peril rating queues N events), supplement-specific event tables (Rivendell etc.), per-hero Fatigue distribution (single-character app only tracks this hero). |
| Council | ~90% | ✅ Full Council tab added. Resistance 3/6/9, Audience Attitude ±1d, Introduction roll sets Time Limit, Interaction phase with 5 skill buttons + Roleplay Bonus picker, auto-end at Resistance met or Time Limit exhausted, Success/Failure/Success-with-Woe outcomes, roll log. Still missing: support-action integration within a council (any player can support the speaker by spending 1 Hope — currently handled via the dice tab's Receive Support); supplement-specific council bonuses; persistent council history across multiple councils per Adventuring Phase (current resets cleanly between councils). |
...
| Skill Endeavours | ~90% | ✅ Full Skill Endeavour section added to the Council tab. Resistance 3/6/9, Time Limit 3/4/5/6, Risk Level Standard/Hazardous/Foolish with branching outcomes, full 18-skill picker, Roleplay Bonus, auto-end at Resistance met or Time Limit exhausted. Still missing: auto-application of Failure-with-Woe consequences (currently flagged in log only — player narrates and adjusts Endurance/Fatigue manually); per-task progress persistence across the Adventuring Phase. |
| Resting/Healing | ~85% | ✅ Short Rest + Prolonged Rest buttons added with full RAW behaviour (Wounded reduction, 0 Hope → +1 on PR, Safe Haven Fatigue clearing). First Aid HEALING roll with day-countdown. Still missing: rest-frequency enforcement (1 short + 1 prolonged per day — currently no day-tracker prevents repeated tapping); auto-decrement of injury day-count over rest events. |
