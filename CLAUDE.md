...
4. `<section.panel>` — **12 tabs**: Character / Skills / Combat / Journey / Council / Gear / Dice / **Oracle** / **Band** / **Battle** / **Chronicle** / Build. Oracle is shown in any solo mode (`isSolo()` in `refreshStriderUI`); **Chronicle is Strider-Mode-only** (`char.striderMode` — visible in a Moria campaign only if Strider Mode is also on, NOT in plain Moria Solo). **Band & Battle are Moria-only** (`char.moriaMode`). Journey & Council are always present. (Tab defaults to `display:none`; `refreshStriderUI` owns its visibility.)
