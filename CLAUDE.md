...
- **`character-tracker.html`**: ~12,201 lines / ~710 KB (includes a ~20 KB vendored QR library in its own `<script>` block)
- **`sw.js` `CACHE_VERSION`**: `tor2e-v59` (bump on every deploy)
